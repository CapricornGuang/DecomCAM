...
class DecomCAM(GradCAM):
    ...
    def get_attn_maps(self, input: torch.Tensor, target: torch.Tensor, image_np: np.ndarray):
        ...
        #alpha = (act*F.relu(grad)).mean(dim=(2, 3), keepdim=True)
        #grad_p_cam = torch.sum(act * alpha, dim=1, keepdim=True)
        #grad_p_cam = torch.clamp(grad_p_cam, min=0)
        #grad_p_cam = torch.sum(act * alpha, dim=1, keepdim=True)#.detach().cpu().numpy()
        #grad_p_cam = F.interpolate(
        #        grad_p_cam,
        #        input.shape[2:],
        #        mode='bicubic',
        #        align_corners=False).squeeze().squeeze().detach().cpu().numpy()
        #self.current_output['attn_map'] += grad_p_cam
